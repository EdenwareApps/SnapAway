...
export class CustomTitlebar extends ThemeBar {
	...
	private loadEvents() {
		...
		// Get initial maximize state using our custom logic
		ipcRenderer.invoke('get-current-state').then((state: any) => {
			if (state && typeof state.maximized === 'boolean') {
				this.onDidChangeMaximized(state.maximized)
			}
		}).catch((error) => {
			console.warn('[Titlebar] Failed to get initial maximize state:', error)
			// Fallback to default Electron method
			this.onDidChangeMaximized(ipcRenderer.sendSync('window-event', 'window-is-maximized'))
		})

		// Listen to our custom maximize state change event instead of the default one
		ipcRenderer.on('window-maximized-changed', (event, isMaximized: boolean) => {
		...
		if (maximizable) {
			addDisposableListener(this.controls.maximize, EventType.CLICK, () => {
				// Use our custom toggle-maximize function instead of the default window-event
